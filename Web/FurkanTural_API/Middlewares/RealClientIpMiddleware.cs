...
///
/// Neden hazır <c>UseForwardedHeaders</c> değil: o middleware yalnızca X-Forwarded-* bilir,
/// Cloudflare'in kanonik <c>CF-Connecting-IP</c> başlığını tanımaz ve KnownProxies listesini
/// yapılandırmadan loopback dışına çıkmaz.
///
/// GÜVENLİK: Başlıklar YALNIZCA istek güvenilir bir proxy ağından geldiyse dikkate alınır.
/// Aksi halde origin'i doğrudan bulan biri CF-Connecting-IP uydurup log tablosunu zehirleyebilir
/// ve Turnstile'ın remoteip kontrolünü yanıltabilirdi. Güvenilmeyen kaynakta başlıklar yok sayılır.
///
/// ⚠️ İKİZ DOSYA: Aynı mantığın bir kopyası
/// <c>Presentation/FurkanTural_Chat/Middlewares/RealClientIpMiddleware.cs</c> içindedir
/// (Chat, tarayıcı IP'sini gövdeyle API'ye relay eder). Birini değiştirirken diğerini de güncelleyin.
/// </summary>
