...
public class ProjectImageController(IProjectImageService projectImageService, IFileService fileService) : JwtBaseController
{
    ...
    /// <summary>Proje görselini ID ile getir</summary>
    [HttpGet("{id:int}")]
    ...
    /// <summary>Tüm proje görsellerini listele</summary>
    [HttpGet]
    [Authorize(Policy = "VisitorOrAbove")]
    public async Task<IActionResult> GetAll(CancellationToken cancellationToken)
        => ToActionResult(await _projectImageService.GetAllAsync(cancellationToken));

    /// <summary>Admin için tüm proje görsellerini listele (soft-deleted dahil)</summary>
    [HttpGet("admin")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<IActionResult> GetAllForAdmin(CancellationToken cancellationToken)
        => ToActionResult(await _projectImageService.GetAllForAdminAsync(cancellationToken));

    /// <summary>Proje görselini yönetici paneli için ID ile getir (silinmiş dahil)</summary>
    ...
    /// <summary>Belirli bir projeye ait görselleri getir</summary>
    ...
    /// <summary>Proje görseli yükle ve kaydet</summary>
    [HttpPost]
    [Authorize(Policy = "AdminOnly")]
    ...
    /// <summary>Proje görselini güncelle</summary>
    [HttpPut]
    [Authorize(Policy = "AdminOnly")]
    ...
    /// <summary>Proje görselini sil</summary>
    [HttpDelete("{id:int}")]
    [Authorize(Policy = "AdminOnly")]
    ...
    /// <summary>Yönetici paneli için süzülmüş ve sayfalı proje görseli listesi</summary>
    [HttpGet("admin/paged")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<IActionResult> GetAdminPaged(
        ...
        => ToActionResult(await _projectImageService.GetAllForAdminPagedAsync(
            AdminListQuery.From(search, isActive, isDeleted, dateFrom, dateTo, pageNumber, pageSize), isCover, projectId, cancellationToken));

    /// <summary>Yönetici paneli için proje görseli durum sayaçları; süzgeçler sayfalı listeyle aynıdır</summary>
    ...
    public async Task<IActionResult> GetAdminCounts(
        ...
        [FromQuery] int? projectId,
        CancellationToken cancellationToken = default)
        => ToActionResult(await _projectImageService.GetAdminStatusCountsAsync(
            AdminListQuery.From(search, isActive, isDeleted, dateFrom, dateTo), isCover, projectId, cancellationToken));
    ...
    public async Task<IActionResult> Bulk([FromBody] BulkActionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return ToActionResult(await _projectImageService.BulkAsync(action, request.Ids ?? [], SortUserId(), cancellationToken));
    }
}
