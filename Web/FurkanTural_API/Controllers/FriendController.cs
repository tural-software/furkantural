...
public class FriendController(IUserFriendService userFriendService) : JwtBaseController
{
    ...
    /// <summary>Yönetici paneli için süzülmüş ve sayfalı arkadaşlık listesi</summary>
    [HttpGet("admin/paged")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<IActionResult> GetAdminPaged(
        ...
        => ToActionResult(await _userFriendService.GetAllForAdminPagedAsync(
            AdminListQuery.From(search, isActive, isDeleted, dateFrom, dateTo, pageNumber, pageSize), statusCode, cancellationToken));

    /// <summary>Yönetici paneli için arkadaşlık durum sayaçları; süzgeçler sayfalı listeyle aynıdır</summary>
    ...
    public async Task<IActionResult> GetAdminCounts(
        ...
        [FromQuery] string? statusCode,
        CancellationToken cancellationToken = default)
        => ToActionResult(await _userFriendService.GetAdminStatusCountsAsync(
            AdminListQuery.From(search, isActive, isDeleted, dateFrom, dateTo), statusCode, cancellationToken));
    ...
    public async Task<IActionResult> Bulk([FromBody] BulkActionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return ToActionResult(await _userFriendService.BulkAsync(action, request.Ids ?? [], SortUserId(), cancellationToken));
    }
}