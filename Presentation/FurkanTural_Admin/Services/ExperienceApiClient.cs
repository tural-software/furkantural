...
public class ExperienceApiClient(HttpClient httpClient, ILogger<ExperienceApiClient> logger) : IExperienceApiClient
{
    ...
    public async Task<(IReadOnlyList<ExperienceAdminDto> Rows, int TotalFiltered)> GetAdminPagedAsync(AdminListRequest request, string token, CancellationToken ct = default)
    {
        try
        {
            using var httpRequest = new HttpRequestMessage(HttpMethod.Get, request.ToQueryString("/api/v1/experience/admin/paged", paged: true));
            ...
            var wrapper = await response.Content.ReadFromJsonAsync<PagedApiResult<ExperienceAdminDto>>(JsonOptions, ct);
            var rows = wrapper?.Data?.ToList().AsReadOnly() ?? (IReadOnlyList<ExperienceAdminDto>)[];
    ...
    public async Task<StatusCountsModel?> GetAdminCountsAsync(AdminListRequest request, string token, CancellationToken ct = default)
    {
        try
        {
            using var httpRequest = new HttpRequestMessage(HttpMethod.Get, request.ToQueryString("/api/v1/experience/admin/counts", paged: false));
            httpRequest.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);

            using var response = await _httpClient.SendAsync(httpRequest, ct);
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("Deneyim sayaçları alınamadı: {Status}", (int)response.StatusCode);
                return null;
            }
        ...
        {
            _logger.LogWarning(ex, "Deneyim sayaç uç noktasına erişilemedi.");
            return null;
        }
        catch (TaskCanceledException)
        {
        ...
        {
            _logger.LogError(ex, "Deneyim sayaçları alınırken beklenmeyen hata oluştu.");
            return null;
        }
    }
    ...
    public async Task<BulkResultModel?> BulkAsync(string action, IReadOnlyList<int> ids, string token, CancellationToken ct = default)
    {
        try
        {
            using var httpRequest = new HttpRequestMessage(HttpMethod.Post, "/api/v1/experience/admin/bulk")
            {
