/**
 * Skill Detail Modal — config + page binding
 *
...
(function () {
        ...
        sections: [
            {
                ...
                fields: [
                    ...
                    },
                    {
                        label: 'Yetkinlik',
                        icon: 'hash-icon',
                        value: function (r) { return r.proficiency + ' / 100'; },
                        badgeVariant: function (r) {
                            return r.proficiency >= 71 ? 'success' : r.proficiency >= 41 ? 'warning' : 'danger';
                        }
                    },
                    {
                        label: 'Aktif',
    ...
    function buildCreateConfig(onSuccess) {
        return {
            ...
            description: 'Yeni bir beceri kaydı oluşturun.',
            submitUrl: '/Skill/Create',
            submitLabel: 'Ekle',
            fields: SKILL_FORM_FIELDS,
            onSuccess: onSuccess
        };
    }
    ...
    function buildEditConfig(id, onSuccess) {
        return {
            title: 'Beceri Düzenle',
            description: 'Mevcut beceri kaydını güncelleyin.',
            submitUrl: '/Skill/Update/' + id,
    ...
    function reloadTable() {
        ...
        fetch('/Skill/TablePartial?' + params.toString(), {
        ...
        .then(function (html) {
            var section = document.getElementById('skill-table-section');
            if (!section) return;
    ...
    function bindAll() {
        ...
        document.querySelectorAll('.ft-view-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                DetailModal.open(SkillDetailConfig, record, function (key) {
                    if (key === 'edit') {
                        DetailModal.close();
