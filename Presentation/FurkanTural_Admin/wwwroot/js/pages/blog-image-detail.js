/**
 * BlogImage Detail Modal — config + page binding
 *
...
(function () {
    ...
    /* ── Detail modal config ──────────────────────────────── */
    var BlogImageDetailConfig = {
        title: 'Kayıt Detayı',
        description: 'Seçilen blog görseli kaydına ait detaylar',
                ...
                fields: [
                    {
                        label: 'Blog ID',
                        icon: 'hash-icon',
                        value: function (r) { return String(r.blogId); }
                    },
                    {
                        label: 'Kapak Görseli',
                        icon: 'shield',
    ...
    /* ── Form field configs ───────────────────────────────── */
    var _blogOptions = null;

    function fetchBlogOptions(callback) {
        if (_blogOptions !== null) { callback(_blogOptions); return; }
        fetch('/BlogImage/BlogOptions', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(function (r) { return r.ok ? r.json() : []; })
            .then(function (data) { _blogOptions = data || []; callback(_blogOptions); })
            .catch(function () { _blogOptions = []; callback([]); });
    }

    function buildBlogIdField(options) {
        return {
            name: 'blogId',
            label: 'Blog',
            type: 'searchable-select',
            required: true,
            placeholder: 'Blog seçin veya arayın...',
            options: options
        };
    }
    ...
    function buildCreateConfig(options, onSuccess) {
        return {
            title: 'Yeni Blog Görseli Ekle',
            description: 'Yeni bir blog görseli kaydı oluşturun.',
            submitUrl: '/BlogImage/Create',
            submitLabel: 'Ekle',
    ...
    function buildEditConfig(id, options, onSuccess) {
        return {
            title: 'Blog Görselini Düzenle',
            description: 'Mevcut blog görseli kaydını güncelleyin.',
            submitUrl: '/BlogImage/Update/' + id,
    ...
    function readRows() {
        var el = document.getElementById('__blog-image-rows-json');
        if (!el) return [];
        try { return JSON.parse(el.textContent || '[]'); } catch (e) { return []; }
    }

    function reloadTable() {
        var meta = window.__blogImageMeta || {};
        ...
        fetch('/BlogImage/TablePartial?' + params.toString(), {
        ...
        .then(function (html) {
            var section = document.getElementById('blog-image-table-section');
            if (!section) return;
    ...
    function openEditModal(record) {
        fetchBlogOptions(function (options) {
            FormModal.open(
    ...
    function bindAll() {
        ...
        document.querySelectorAll('.ft-view-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                DetailModal.open(BlogImageDetailConfig, record, function (key) {
                    if (key === 'edit') {
                        DetailModal.close();
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var addBtn = document.getElementById('blog-image-add-btn');
        if (addBtn) {
