/**
 * ProjectImage Detail Modal — config + page binding
 *
...
(function () {
    ...
    var ProjectImageDetailConfig = {
        title: 'Kayıt Detayı',
        description: 'Seçilen proje görseli kaydına ait detaylar',
                ...
                fields: [
                    {
                        label: 'Proje ID',
                        icon: 'hash-icon',
                        value: function (r) { return String(r.projectId); }
                    },
                    {
                        label: 'Kapak Görseli',
                        icon: 'shield',
    ...
    function fetchProjectOptions(callback) {
        if (_projectOptions !== null) { callback(_projectOptions); return; }
        fetch('/ProjectImage/ProjectOptions', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(function (r) { return r.ok ? r.json() : []; })
            .then(function (data) { _projectOptions = data || []; callback(_projectOptions); })
            .catch(function () { _projectOptions = []; callback([]); });
    }

    function buildProjectIdField(options) {
        return {
            name: 'projectId',
            label: 'Proje',
    ...
    function buildCreateConfig(options, onSuccess) {
        return {
            title: 'Yeni Proje Görseli Ekle',
            description: 'Yeni bir proje görseli kaydı oluşturun.',
            submitUrl: '/ProjectImage/Create',
            submitLabel: 'Ekle',
    ...
    function buildEditConfig(id, options, onSuccess) {
        return {
            title: 'Proje Görselini Düzenle',
            description: 'Mevcut proje görseli kaydını güncelleyin.',
            submitUrl: '/ProjectImage/Update/' + id,
    ...
    function readRows() {
        var el = document.getElementById('__project-image-rows-json');
        if (!el) return [];
        try { return JSON.parse(el.textContent || '[]'); } catch (e) { return []; }
    }

    function reloadTable() {
        var meta = window.__projectImageMeta || {};
        ...
        fetch('/ProjectImage/TablePartial?' + params.toString(), {
        ...
        .then(function (html) {
            var section = document.getElementById('project-image-table-section');
            if (!section) return;
    ...
    function openEditModal(record) {
        fetchProjectOptions(function (options) {
            FormModal.open(
    ...
    function bindAll() {
        ...
        document.querySelectorAll('.ft-view-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                DetailModal.open(ProjectImageDetailConfig, record, function (key) {
                    if (key === 'edit') {
                        DetailModal.close();
